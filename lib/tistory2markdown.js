...
function parseBody(article) {
   var article = article.replace(/<br \/>/g, '').replace(/&nbsp;/g, ' ').replace(/<\/div>/g, '</div>\n');
   article = article.replace(/<div>/g, '').replace(/<\/div>/g,'');
   article = toMarkdown(article);
   //article = stringEx.stripHtmlTags(article);
...
parser.on('end', function(result){
   ...
   posts.forEach(function(post){
      ...
      console.log('origin :', post.content.length, 'byte, markdown :', article.length, 'byte');
